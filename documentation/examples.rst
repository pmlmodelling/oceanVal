...
- `lon_lim`: The longitude limits for the validation. This is a list of two values, e.g. [-10, 10] for 10 degrees west to 10 degrees east. 
- `lat_lim`: The latitude limits for the validation. This is a list of two values, e.g. [40, 50] for 40 to 50 degrees north.

If you want to speed things up, you can set the number of cores to use using the `cores` argument, which is 6 by default.
...
.. code:: ipython3
   ...
   import ecoval
   ecoval.matchup("/foo/bar",   cores = 6, start = 2000, end = 2010)
...
A starting point for AMM7 validations
------------------------------------------------

If you want to validate the AMM7 domain, and want a "maximalist" starting point, the following will do:

.. code:: ipython3

   ecoval.matchup("/foo/bar", start = 2000, end = 2010, surface = {"gridded": ["ammonium", "chlorophyll", "co2flux", "doc", "kd", "nitrate", "oxygen", "pco2", "phosphate", "poc", "salinity", "silicate", "temperature"],
                                                                   "point": ["alkalinity", "ammonium", "chlorophyll", "doc", "mesozoo", "nitrate",
                                                                             "oxygen", "pco2",  "ph", "phosphate", "phosphorus", "poc", "salinity", "silicate", "temperature"]},
                                                                   bottom = ["alkalinity", "ammonium", "chlorophyll",  "nitrate",
                                                                             "oxygen",   "ph", "phosphate", "phosphorus", "poc", "salinity", "silicate", "temperature"]},
                                                                     benthic = ["benbio", "susfrac", "oxycons", "carbon"]
                   cores = 6, pft = True)

```

This will cover all available surface, bottom and benthic variables, as well as plankton functional types. Just delete what you do not want to validate.
...
If you want to work outside PML, you will need to make transfer a copy of the following directory to your local machine: /data/thaumus2/scratch/common/ecoval.
